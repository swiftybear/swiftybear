$(document).ready(function(){
	...
	*/
	$("#search_btn").click(function(){
		...
		else {  $("#get_product").html("<h3>No Input. Please Try again !</h3>");  }
	})
var x=0;
...
$("#get_product").html("Data Tables ! ");
		...
		$.ajax({
			...
			success	:function(data){
				if(data == "login_success"){
				   ...
				   $("#e_msg").html(    "<span style='color:green;'>Login success</span>  <script>jQuery.noConflict();$('#myModalz').modal('show');</script> " );
		    setTimeout('window.location.href = "x.php"',1850)   ;


				/* old code  	window.location.href = "x.php"  -- no delay*/
		...
		$.ajax({
			...
			success	:function(data){
				if(data == "login_success"){
					 ...
					 $("#e_msg").html(    "<span style='color:green;'>Login success</span>  <script>jQuery.noConflict();$('#myModalz').modal('show'); $('#myModal3').modal('hide'); </script> " );
				setTimeout('window.location.href = "x.php"',1999)   ;
		...
		$.ajax({
			...
			success : function(data){
				...
				if (data == "register_success") {
					$("#e_msg").html(    "<span style='color:green;'>Register success</span>  <script>jQuery.noConflict();$('#myModalz').modal('show');</script> " );
			 setTimeout('window.location.href = "login_form.php"',2350)   ;
				}else{
	...
	*/
	$("body").delegate(".qty","keyup",function(event){
		...
		$('.net_total').html("Expected cost total: ₱ " +(net_total.toFixed(2)));
